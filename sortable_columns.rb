  ...
  module SortableColumns
    ...
    module HelperMethods
      ...
      def sortable_column(title, options = {})
        ...
        cur_page = (!params[:page].blank?) ? params[:page] : 1

        # Already sorted?
        if pp[:column] == o[:column].to_s
          pcs << link_to(title, params.merge({conf[:sort_param] => [("-" if pp[:direction] == :asc), o[:column]].join, conf[:page_param] => cur_page}), html_options)       # Opposite sort order when clicked.
          ...
          # Not sorted.
          pcs << link_to(title, params.merge({conf[:sort_param] => [("-" if o[:direction] != :asc), o[:column]].join, conf[:page_param] => cur_page}), html_options)
        end
